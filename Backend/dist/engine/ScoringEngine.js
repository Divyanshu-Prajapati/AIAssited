...
class ScoringEngine {
    static calculateScore(question, execResult, promptQualityScore, planQualityScore, code) {
        ...
        const overallScore = Math.round((correctnessScore + timeComplexityScore + spaceComplexityScore + edgeCaseScore + codeQualityScore + reasoningScore) * 10) / 10;
        // Determine status
        ...
        if (passRatio === 1)
            strengths.push('Passed all visible and hidden test cases.');
        if (timeComplexityScore === 2)
            strengths.push(`Achieved optimal time complexity (${question.expectedTimeComplexity}).`);
        if (reasoningScore >= 0.8)
            strengths.push('Demonstrated strong problem understanding and algorithm planning.');
        if (passRatio < 1)
            areasToImprove.push(`Failed ${totalTests - passedTests} out of ${totalTests} test cases.`);
        if (timeComplexityScore < 2)
            areasToImprove.push(`Optimize algorithm time complexity to reach ${question.expectedTimeComplexity}.`);
        if (reasoningScore < 0.7)
            areasToImprove.push('Improve prompt specificity and technical plan breakdown.');
